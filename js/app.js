const SENSORS={'1/4':{w:3.2,h:2.4,label:'1/4″'},'1/3':{w:4.8,h:3.6,label:'1/3″'},'1/2.5':{w:5.76,h:4.29,label:'1/2.5″'},'1/2':{w:6.4,h:4.8,label:'1/2″'},'1/1.8':{w:7.2,h:5.3,label:'1/1.8″'},'2/3':{w:8.8,h:6.6,label:'2/3″'},'1':{w:12.8,h:9.6,label:'1″'},'4/3':{w:17.3,h:13,label:'4/3″'},'aps-c':{w:23.6,h:15.7,label:'APS-C'}};
const CAMERA_RECS=[{name:'130万',w:1280,h:1024,total:1.3e6},{name:'200万',w:1920,h:1080,total:2.1e6},{name:'500万',w:2592,h:1944,total:5.0e6},{name:'600万',w:3072,h:2048,total:6.3e6},{name:'1200万',w:4096,h:3000,total:12.3e6},{name:'2000万',w:5472,h:3648,total:20e6}];
...
].map(g=>'<div class="rp-group"><div class="rp-group-title">'+g.t+'</div><div class="rp-group-grid">'+g.i.map(p=>'<div class="rp-cell"><span class="rp-label">'+p[0]+'</span><span class="rp-value">'+p[1]+'</span></div>').join('')+'</div></div>').join('');
}
...
inputIds.forEach(id=>{const el=document.getElementById(id);if(el)el.addEventListener('input',calcAll);});
document.getElementById('btn-refresh').addEventListener('click',()=>{calcAll();document.getElementById('overview')?.scrollIntoView({behavior:'smooth',block:'start'});});
const backTop=document.getElementById('back-top');
window.addEventListener('scroll',()=>{backTop.classList.toggle('visible',window.scrollY>window.innerHeight*0.6);});
